...
export default function FoodFraudDefenseTab({ t, qualData, setQualData, supplierId, saveImmediate }) {
  ...
  const Section = ({ title, sectionKey, section }) => {
    ...
    <div className={`bg-slate-50 p-10 rounded-[3rem] border shadow-inner space-y-6 ${issues.length ? "border-amber-400" : "border-slate-200"}`}>
      ...
      <div className="space-y-2">
        ...
        {section.files.length === 0 && <p className="text-xs text-slate-400 font-bold">Nessun file caricato</p>}
      ...
      <div className="space-y-1 max-w-md">
        <label className="text-[10px] font-black uppercase text-slate-500">
          Applicabile a {appliesToMissing && <span className="text-red-500">*obbligatorio</span>}
        </label>
        <input
          ...
          placeholder="Se il documento riguarda solo prodotti/famiglie specifiche, indicali qui..."
        />
      </div>
    </div>
    );
  ...
  return (
    <div className="space-y-12">
      ...
      <Section title="Food Fraud" sectionKey="foodFraud" section={fraud} />
      <Section title="Food Defense" sectionKey="foodDefense" section={defense} />
    </div>
  );
}
